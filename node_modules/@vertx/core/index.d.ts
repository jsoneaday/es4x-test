...
export abstract class CompositeFuture extends Future<CompositeFuture> {
   ...
   */
  compose<U>(successMapper: (arg: CompositeFuture) => Future<U>, failureMapper: (arg: Error) => Future<U>) : Future<U>;
  ...
  onFailure(handler: ((res: Error) => void) | Handler<Error>) : CompositeFuture;
...
export abstract class EventBus implements Measured {
  ...
  /**
   * Like {@link EventBus#send} but specifying <code>options</code> that can be used to configure the delivery.
   */
  ...
  /**
   * Like {@link EventBus#request} but specifying <code>options</code> that can be used to configure the delivery.
   */
   ...
   * <p>
   * The returned sender will invoke the {@link EventBus#send}
   ...
   * <p>
   * The returned publisher will invoke the {@link EventBus#publish}
...
export abstract class Future<T> implements PromiseLike<T> {
   ...
   * <br/>
   */
  onFailure(handler: ((res: Error) => void) | Handler<Error>) : Future<T>;
   ...
   * If any mapper function throws an exception, the returned future will be failed with this exception.<p>
   */
  compose<U>(successMapper: (arg: T) => Future<U>, failureMapper: (arg: Error) => Future<U>) : Future<U>;
...
export abstract class HttpClientRequest implements ReadStream<HttpClientResponse>, WriteStream<Buffer> {
   ...
   * written to it.
   * <p>
   * This is normally used to implement HTTP 100-continue handling, see {@link HttpClientRequest#continueHandler} for
   * more information.
   */
...
export abstract class HttpServer implements Measured {
   ...
   * instances of {@link HttpServerRequest} will be created and passed to the stream {@link ReadStream#handler}.
   */
   ...
   * new {@link ServerWebSocket} instance will be created and passed to the stream {@link ReadStream#handler}.
   */
...
export abstract class Message<T> {
  ...
  /**
   * Like {@link Message#replyAndRequest} but specifying <code>options</code> that can be used
...
export abstract class ServerWebSocket implements WebSocketBase {
   ...
   * {@link WebSocketBase#binaryMessageHandler}, but the buffer will be converted to a String first
   */
...
export abstract class Vertx implements Measured {
  ...
  /**
   * Set a default exception handler for {@link Context}, set on {@link Context#exceptionHandler} at creation.
   */
